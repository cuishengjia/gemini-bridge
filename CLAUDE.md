...
Skill 本体位于 `skills/ask-gemini-cli/`。该目录之外的内容都是支撑材料（设计文档、Gemini CLI 参考文档、本文件），**不随 skill 发布**。
...
以下命令默认 `cwd = skills/ask-gemini-cli/`。仓库没有 `pyproject.toml` / `conftest.py` —— 测试文件自己往 `sys.path` 注入路径，所以直接在 skill 目录下跑 `python3 -m pytest` 即可。
...
ruff check skills/ask-gemini-cli
...
- `skills/ask-gemini-cli/docs/test-report.md` —— 活调用冒烟结果、已知 bug 复盘（含 brace-escape、用户回显两起事故）。
